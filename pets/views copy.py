...
#     """
#     Custom permission to allow creating resources only for authenticated users, but allow read access to everyone.
#     """
#     def has_permission(self, request, view):
#         if request.method in ['GET', 'HEAD', 'OPTIONS']:
#             return True  # Allow read access for everyone
#         if request.method == 'POST':
#             return request.user.is_authenticated  # Allow POST only for authenticated users
...
#         return obj.author == request.user  # Allow write access only if the user is the owner

# class IsSuperUser(BasePermission):
#     """
#     Custom permission that allows only superusers to access the view.
#     """
#     def has_permission(self, request, view):
#         return request.user.is_superuser  # Only superusers can access

# class IsAuthenticatedAndHasRole(BasePermission):
#     """
#     Custom permission to allow access to users who are authenticated and have a specific role.
#     """
#     def has_permission(self, request, view):
...
class PetViewSet(viewsets.ModelViewSet):
    ...
    #exmaple for custom
    #permission_classes = [IsAuthenticatedAndHasRole]  # Only authenticated users with 'admin' role can access
    #permission_classes = [IsSuperUser]  # Only superusers can access this endpoint
    #permission_classes = [IsOwnerOrReadOnly]  # Only the owner can update/delete their pets
    #permission_classes = [IsAuthenticatedOrCreate]  # Allow creating only for authenticated users
    #permission_classes = [IsAuthenticated]  # Requires login for all operations
    #permission_classes = [AllowAny]  # Unrestricted access to everyone
